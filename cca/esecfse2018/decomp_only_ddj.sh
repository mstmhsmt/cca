...
function usage()
{
  echo "usage: ${cmdname} [-a dd|ddmin] [-m 8|16|32|48|64] [-s] [-l N] [-t R] [-u N] [-x] [-c] [-g] PROJ_ID" 1>&2
}
...
if [ ! -d ${CCA_PROJECTS_DIR}/${PROJ_ID_} ]; then
    ...
    tar -C ${CCA_PROJECTS_DIR} -jxvf ${EXAMPLES_DIR}/${PROJ_ID_}.tbz >& /dev/null
fi
...
${DD_DIR}/setup_factbase.py${FB_OPTS} ${PROJ_ID_} && \
date && \
sleep 5 && \
